...
struct board_type {
    ...
    void move(const i32 crane, const i32 to) {
        if(to_col(cranes[crane].pos) < to_col(to)) {
...
signed main() {
...

    {
        ...
        REP(i, N) {
            REPD(j, left, N - 1) {
                board.move(i, to_id(i, 0));
                board.carry(i, to_id(i, 0), to_id(i, j));
            }
        }
    }
    ...
    while(!queue.empty()) {
        auto cost = [&](i32 id) -> i32 {
            i32 dist = distance(board.pos[id], board.cranes[0].pos);;
        ...
        const auto next = queue.back(); queue.pop_back();
        ...
        debug_("export");
        board.carry(0, board.pos[next], to_goal(next));
