...
int les[BLOCK*BLOCK], les_block[BLOCK];
...
int gre[BLOCK*BLOCK], gre_block[BLOCK];
int numGre(int x) { return gre[x]+gre_block[x/BLOCK]; }
void insGre(int x) {
	for (;x%BLOCK;) gre[--x] ++;
	for (x /= BLOCK; x; ) gre_block[--x] ++;
}
void ins(int x) { insLes(x), insGre(x); }
