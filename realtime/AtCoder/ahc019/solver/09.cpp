...
using random = lib::random_engine<lib::xorshift>;
random rng;

struct timer : private internal::uncopyable {
...
static std::array<vector<polycube>,2> Put_, PutBest;
static polycube_collector Ps_;

void save() { Put_ = Put, Ps_ = Ps; }
void rollback() { Put = std::move(Put_), Ps = std::move(Ps_); }
bool update() {
    if(MinCost > Ps.cost()) {
        MinCost = Ps.cost();
        PutBest = Put;
...
void split(
    const size_t n,
    const typename polycube::iterator first,
    const typename polycube::iterator middle,
    const typename polycube::iterator last
) {
    const auto a = polycube(first, middle);
    const auto b = polycube(middle, last);
...
void split_randomly(const size_t n) {
    ...
    const auto itr = Put[n][p].begin() + lib::rng(Put[n][p].size());

    split(n, Put[n][p].begin(), itr, Put[n][p].end());
}

void remove_randomly(const size_t n) {
    point3d p;
    do {
        p = lib::rng(V);
    } while(!Put[n][p]);
    ...
    remove(n, Put[n][p].begin(), itr,  Put[n][p].end());
}
...
void finalize() {
    ...
    REP(n, 2) {
        REP(p, V) {
            if(!PutBest[n][p]) continue;
            if(Res[n][p]) continue;

            // debug(point3d{p}, PutBest[n][p]);

            size_t id = cubes[n].push(PutBest[n][p])+1;
...
void solve() {
    ...
    DEBUG("--- modifying ---");
    {
        constexpr i64 START_TEMPERATURE = 100000000L;
        constexpr i64 END_TEMPERATURE = 10000000L;

        int loops = 0;
        while(not timer.expired()) {
            const ld temperature = timer.progress() * (END_TEMPERATURE - START_TEMPERATURE) + START_TEMPERATURE;

            const cost_t prev_cost = Ps.cost();
            save();
            ...
            REP(3) {
                ...
                if(t == 0) connect_randomly(n);
                if(t == 1) split_randomly(n);
                if(t == 2) remove_randomly(n);
            }

            const cost_t cur_cost = Ps.cost();
            const cost_t cost_diff = cur_cost - prev_cost;

            const ld probability_of_acceptance = std::exp(-cost_diff / temperature);
            // debug(cost_diff, temperature, timer.progress(), probability_of_acceptance);

            if(probability_of_acceptance >= (ld)lib::rng() / lib::random::max()) {
                if(update()) {
                    std::clog << cur_cost << " (" << cost_diff << ") " << std::endl;
                    if constexpr(DEVENOPMEMT_MODE) log();
                }
            }
            else {
                rollback();
        ...
        std::clog << "iterations: " << loops << std::endl;
    }
