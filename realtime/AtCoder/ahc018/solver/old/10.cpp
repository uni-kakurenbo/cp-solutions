...
struct solver : lib::uncopyable {
    ...
    void move(lib::point start, const lib::point goal) {
        ...
        {
            std::deque<std::pair<lib::point,lib::size_t>> que;
            std::array<lib::point,lib::S> prev; prev.fill({ -1, -1 });

            que.emplace_back(start, 0), prev[start.id()] = { 0, -1 };

            while(!que.empty()) {
                auto [ p, d ] = que.front(); que.pop_front();
                ...
                ITR(di, dj, DIRS4) {
                    auto q = p + lib::point{ di, dj };

                    if(!q) continue;

                    if(prev[q.id()].row >= 0) continue;

                    if(this->F->broken.test(q.id())) que.emplace_front(q, d);
                    else que.emplace_back(q, d+1);
