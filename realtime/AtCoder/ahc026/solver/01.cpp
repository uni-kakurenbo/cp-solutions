...
signed main() {
    ...
    while(STORAGE.removed < N) {
        ...
        std::array<int, M> costs;
        REP(i, M) {
            if(i == STORAGE.stack(id)) CONTINUE(costs[i] = INF32);

            auto copied = STORAGE;
            copied.move(STORAGE.above(id), i);

            const int sz = std::ranges::ssize(copied.data[i]);
            costs[i] = 0;
            REP(l, sz) REP(r, sz) {
                if(l < r and copied.data[i][l] < copied.data[i][r]) {
                    costs[i] += (copied.data[i][r] - copied.data[i][l]);
                }
            }
        }

        int to = std::ranges::min_element(costs) - std::ranges::begin(costs);
        STORAGE.move<true>(STORAGE.above(id), to);
        // break;
    }
