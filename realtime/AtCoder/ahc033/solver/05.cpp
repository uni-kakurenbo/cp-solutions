...
signed main() {
    ...
    while(true) {
        if(state.empty(0)) {
            ...
            if(state.next_pos(1) == pos || move_request[0]) {
                move_request[0] = false;
                if(to_row(pos) >= N / 2) state.move(0, pos - N);
                else state.move(0, pos + N);
            }
            ...
            if(board.pos[next] < 0) {
                ...
                if(board.data[state.cranes[1].pos] == next) {
                    move_request[1] = true;
                }
        ...
        if(state.empty(1)) {
            ...
            if(state.next_pos(0) == pos || move_request[1]) {
                move_request[1] = false;
        ...
        debug(board.data, state.locked, queue, state.schedule);
        state.advance();
        // state.output();
    }
