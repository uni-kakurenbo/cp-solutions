...
struct Field {
    ...
    bool can_connect(int row, int col, int dir, int *res_pos = nullptr, bool *cutted = nullptr) const {
        int nrow = row + DR[dir];
        int ncol = col + DC[dir];
        while(0 <= nrow && nrow < N && 0 <= ncol && ncol < N) {
            if(grid[nrow][ncol] == grid[row][col]) {
                ...
                if(cutted != nullptr) *cutted = grid[nrow][ncol] == 'x';
                return false;
            }
            nrow += DR[dir];
            ncol += DC[dir];
...
struct Connector {
    static constexpr int DIRS[2] = { 0, 1 };

    const Converter *converter;

    Connector(const Converter &converter) : converter(&converter) {}

    void connect_all(Field *target, UnionFind *clusters, bool recording = false) {
...
int main() {
    ...
    const int frequency = MOVE_LIMITS[density];
...

    {
        ...
        connector.connect_all(&tmp, &clusters);
    ...
    Field ans = buffer.begin()->field;

    int cnt = 0;
    double start_temp = 1000, end_temp = 100;
    // while(cnt < 10) {
    while((clock() - start) * 1000 < TIME_LIMIT_MS * CLOCKS_PER_SEC) {
        ++cnt;
        int l = buffer.size();
        double temp = start_temp + (end_temp - start_temp) * ((clock() - start) * 1000) / (TIME_LIMIT_MS * CLOCKS_PER_SEC);
        for(int i=0; i<l; ++i) {
            const State *node = &buffer[i];
            const int gen = node->gen;
            const int pre_score = node->score;
            ...
            for(int j=0; j<Alpha; ++j) {
                ...
                double prob = exp((score-pre_score)/temp);
                // debug(prob, score - pre_score);

                if(prob > (random_engine.generate() % INF32) / (double)INF32 ) {
        ...
        if(buffer.begin()->score > score_max) {
            ...
            ans = buffer.begin()->field;
            #ifdef LOCAL_JUDGE
                Field tmp = buffer.begin()->field;
                UnionFind clusters(N*N);
                connector.connect_all(&tmp, &clusters, true);
    ...
    {
        UnionFind clusters(N*N);
        connector.connect_all(&ans, &clusters, true);
    }
