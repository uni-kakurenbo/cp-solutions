...
struct scheduler {
    ...
    std::optional<std::vector<i32>> root_vacancy(const i32 crane, const i32 from, const i32 time) {
        std::queue<i32> que;
        vector<i32> dist(N * N, INF32), prev(N * N, -2);
    ...
    auto get_motion(const i32 from, const i32 to) {
        ...
        if(to_row(from) == to_row(to) && to_col(from) - 1 == to_col(to)) return OPERATION::MOVE_LEFT;
        if(to_row(from) == to_row(to) && to_col(from) + 1 == to_col(to)) return OPERATION::MOVE_RIGHT;
        if(to_col(from) == to_col(to) && to_row(from) - 1 == to_row(to)) return OPERATION::MOVE_UP;
        if(to_col(from) == to_col(to) && to_row(from) + 1 == to_row(to)) return OPERATION::MOVE_DOWN;
        assert(false);
    }
    ...
    void _advance(const i32 crane) {
        ...
        switch(schedule[crane].front().op) {
            case OPERATION::PAUSE: ok = _pause(crane); break;
        ...
        REP(crane, crane_num) {
            REP(i, N) {
                if(
                    ...
                    next[crane].first == -1 &&
                    (
                        (i != crane && state.next_pos(i) == pos) ||
                        (board.data[pos] % N == 0 || board.pos[board.data[pos] - 1] == -INF32)
                    )
                ) {
                    state.escape(crane);
                    break;
