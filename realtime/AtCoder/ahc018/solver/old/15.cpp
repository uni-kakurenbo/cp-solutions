...
struct solver : lib::uncopyable {
    ...
    int move(const lib::point start, const lib::point goal) {
        ...
        ITR(p, path) {
            const int res = this->destruct(
                p,
                10 * std::sqrt(this->F->C),
                std::max(2500.0, 10000.0 / (0.01 + std::sqrt(std::min(p.dist_e(goal), p.dist_e(start)))))
            );

            debug(std::max(2500.0, 10000.0 / (0.01 + std::sqrt(std::min(p.dist_e(goal), p.dist_e(start))))));

            if(res == 2) {
                if(std::count(ALL(this->samples), p)) continue;
                debug(p);
                this->samples.push_back(p);
                this->strengths[p.id()].second = 5000;
                return 1;
            }
        }
        return 0;
