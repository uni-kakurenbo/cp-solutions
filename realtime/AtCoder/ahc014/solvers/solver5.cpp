...
struct Board {
  ...
  public:
    ...
    bool find_rectangles(const int px, const int py, vector<Rectangle> *found/*, set<Point> *candidates, int test = 0*/) const {
        ...
        for(int d=0; d<8; ++d) {
            ...
            if(
                ...
                grid.is_valid(tx10, ty10) && grid.is_valid(gx0, gy0)
            ) {
                ...
                if(tx2 == gx0 && ty2 == gy0) {
                    ...
                    if(tx3 == px && ty3 == py ) {
                        if(grid.get(gx0, gy0)) {
                            ...
                            // candidates->emplace(px, py);
                        } else {
                            // candidates->emplace(gx0, gy0);
                        }
                    }
                }
            }
            ...
            if(
                ...
                grid.is_valid(tx11, ty11) && grid.is_valid(gx1, gy1)
            ) {
...
struct Evaluator {
    ...
    uint32_t calc_score(Board &board) {
        double weight_sum = 0;
        for(int x=0; x<N; ++x) for(int y=0; y<N; ++y) {
            if(board.grid.get(x, y)) weight_sum += this->weight(x, y);
        }
...
struct State {
    ...
    set<Point> next_candidates;

    State(const int score, const Board &board, const set<Point> &next) : score(score), board(board), next_candidates(next) {}
...
signed main() {
    ...
    deque<State> buffer;
    {
        ...
        for(int x=0; x<N; ++x) for(int y=0; y<N; ++y) {
            initial_board.find_rectangles(x, y, &found);
        }
        buffer.emplace_back(initial_score, initial_board);
    }
    ...
    while(true) {
        const int times = buffer.size();
        for (int i=0; i<times; ++i) {
            if((clock() - start) > TIME_LIMIT) goto end;

            State current_state = buffer[i];

            vector<Rectangle> found;
            set<Point> next;
            for(const auto [x, y] : current_state.next_candidates) {
                current_state.board.find_rectangles(x, y, &found);
                // if(found.empty() and x == 17 and y == 22) debug(current_state.board.grid.view());
            }
            if(found.empty()) continue;

            for(const Rectangle& rectangle : found) {
                Board next_board = current_state.board;
                next_board.draw(rectangle);
                uint32_t score = evaluator.evaluate(next_board);
                debug(score);
                buffer.emplace_back(score, next_board, next);
            }
        }
        // debug(buffer);
    ...
    auto answer = buffer.begin();
    answer->score = evaluator.calc_score(answer->board);
    print_answer(&cout, &clog, *answer);

    // vector<Rectangle> found;
    // set<Point> next;
    // answer->board.find_rectangles(11, 16, &found, &next, 1);
    // debug(answer->next_candidates.count({11,16}), found, next);
