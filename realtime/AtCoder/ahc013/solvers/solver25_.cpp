...
int main() {
    ...
    while((clock() - start) * 1000 < TIME_LIMIT_MS * CLOCKS_PER_SEC) {
        ...
        sort(buffer.begin(), buffer.end(),
            [](const State &a, const State &b) { return a.score == b.score ? a.field.history.move.size() < b.field.history.move.size() : a.score > b.score;
        });
    ...
    {
        UnionFind clusters(N*N);
        connector.connect_all(&ans->field, &clusters, true);
    }
