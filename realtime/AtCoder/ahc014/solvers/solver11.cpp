...
struct Evaluator {
    ...
    int evaluate(Board &board, double time = 0) {
        int weight_sum = 0;
        for(int x=0; x<N; ++x) for(int y=0; y<N; ++y) {
        int w = this->weight(x, y);
            if(board.grid.get(x, y)) weight_sum += pow(w, 1 + time);
        }
        return weight_sum;
    }
...
signed main() {
    ...
    double start_temperature = 700, end_temperature = 60;
    ...
    while((clock() - start) < TIME_LIMIT) {
        ...
        int type = random_engine.range(2);

        if(type == 0 && state.board.history.size() > 5) {
            int times = floor(random_engine.range(1, 7) / 2);
        ...
        state.score = evaluator.evaluate(state.board, exetime);

        int diff = state.score - previous_state.score;
        ...
        if(diff >= 0) {
            ...
            double random = (double)random_engine.generate() / INF32;
