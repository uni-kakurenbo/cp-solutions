...
struct RandomEngine {
    static constexpr UnsignedInteger MAX = mt19937::max();

    RandomEngine(UnsignedInteger seed = 3141592653UL) : x(seed) {
        engine.seed(seed);
    };

    UnsignedInteger generate() {
        // x ^= x << 13; x ^= x >> 17; x ^= x << 15;
        // return x;
        return engine();
    }

    UnsignedInteger range(UnsignedInteger max) {
        return this->generate() % max;
    }

    UnsignedInteger range(UnsignedInteger min, UnsignedInteger max) {
        dev_assert(min < max);
...
struct Point {
    ...
    pair<Integer, Integer> _debug() const {
        return { x, y };
    }
};
...
constexpr Integer BASE_SCORE = 1000000;
using Score = Integer;
...
struct Evaluator {
    ...
    inline UnsignedInteger weight(const Integer x, Integer y) const {
    ...
    Integer calc_score(Board& board) const {
...
void print_answer(ostream *out, const vector<Rectangle> answer) {
    (*out) << answer.size() << "\n";
...
void print_answer(ostream *out, const vector<Rectangle> answer, ostream *log, const Score score) {
    (*log) << score << "\n";
...
struct Candidate {
    ...
    inline bool operator<(const Candidate& other) const {
        return this->score == other.score ? this->priority < other.priority : this->score < other.score;
    }
    inline bool operator>(const Candidate& other) const {
        return this->score == other.score ? this->priority > other.priority : this->score > other.score;
    }
...
struct BeamSearch {
    ...
    void run(vector<Rectangle> *answer, const Integer width) {
        ...
        Integer time_check_frequency = 1;
        ...
        for (; ; ++loop_count) {
            ...
            if(loop_count % time_check_frequency == 0) {
                if(this->timer->expired()) break;
                if(time_check_frequency >= 2) time_check_frequency /= 2;
            }
            ...
            current.clear();
            visited.clear();
            for (const auto& [rectangle, parent, priority, next_score, next_hash] : next_states) {
                if(visited.count(next_hash)) continue;
                visited.insert(next_hash);
                auto child_ptr = shared_ptr<Node>(new Node(Parent({ rectangle, parent }), Children{}, next_score, next_hash));
...
struct Solver {
    ...
    Score solve(vector<Rectangle> *answer) const {
        ...
        Modifier modifier(N, M);

        Score score = evaluator.evaluate(board);
        Hash hash = board.lines.hash();

        State state(score, hash, board);

        { // Beam Search
            BeamSearch beam_search(timer.reset(), state, modifier);
...
signed main() {
    ...
    Grid inputted_grid(N);
    for (Integer i = 0; i < M; ++i) {
        Integer x, y; cin >> x >> y; inputted_grid.put(x, y);
    }

    const Solver solver(N, M, inputted_grid);
