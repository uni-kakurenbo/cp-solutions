...
signed main() {
    ...
    int initial_score = evaluator.calc_score(initial_board);

    deque<State> buffer;
    buffer.emplace_back(initial_score, initial_board);

    int Alpha = 1;
    int Beta = 1; max(57-N, 1);
    ...
    while(true) {
        ...
        for (int i=0; i<times; ++i) {
            ...
            State current_state = buffer.front();
            ...
            debug(current_state.score);
            debug(found.size());

            for(int j=0, l=min(Alpha,(int)found.size()); j<l; ++j) {
                Board next_board = current_state.board;
                next_board.draw(found[random_engine.range(found.size())]);
                int score = evaluator.calc_score(next_board);
