...
struct Solver {
    ...
    static void connect_all(Field *target, const int type = 0) {
        const int N = target->N;

        int first = 0, adv = 1, dirs[2] = { 0, 1 };
        function<bool(int)> continuing = [&N](int x) { return x < N; };

        if(type == 1) {
            first = N-1, adv = -1, dirs[0] = 2, dirs[1] = 3;
            continuing = [](int x) { return x >= 0; };
        }

        for(int i=first; continuing(i); i += adv) {
            for(int j=first; continuing(j); j += adv) {
                if(target->grid[i][j] != '0' && target->grid[i][j] != 'x') {
                    for(int dir : dirs) {
...
constexpr int MOVE_LIMITS[10] = { 8, 7, 5, 2, 1, 1, 2, 3, 8, 10 };
...
int main() {
    ...
    while((clock() - start) * 1000 < TIME_LIMIT_MS * CLOCKS_PER_SEC) {
        ...
        Field tmp0 = ret;
        Solver::connect_all(&tmp0, 0);
        int score = calc_score(tmp0);

        Field tmp1 = ret;
        Solver::connect_all(&tmp1, 1);
        int score0 = calc_score(tmp1);
        if(score < score0) tmp0 = move(tmp1), score = score0;
        ...
        if(score_max < score) {
            ...
            // print_answer(&of_log, tmp0.history);
