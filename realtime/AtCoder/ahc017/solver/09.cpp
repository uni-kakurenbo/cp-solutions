...
struct evaluator {
  ...
  public:
    ...
    score_type compute_strict_score(const solution& schedule) const noexcept {
        score_type sum = 0;

        for(day_type d=0; d<this->_in.D; ++d) {
            auto costs = this->_in.G.compute_cost_matrix(schedule.edges[d]);
...
struct modifier {
  ...
  public:
    ...
    inline neighbor_type choose(const timer::progress_type &progress) const {
        return 10 - progress * 9;
    }

    inline void apply(solution *const days, const neighbor_type neighbor) const {
        std::unordered_set<day_type> changed_days;

        for(size_type i=0; i<neighbor; ++i) {
            day_type d0 = random(this->_in.D);
            day_type d1 = random(this->_in.D);

            if(d0 == d1) continue;
            if(static_cast<day_type>(days->edges[d1].size()) + 1 > this->_in.K) continue;

            auto e = std::next(days->edges[d0].begin(), random(days->edges[d0].size()));
            days->edges[d1].insert(*e);
            days->edges[d0].erase(e);

            changed_days.insert(d0), changed_days.insert(d1);
        }


        for(const day_type day : changed_days) {
