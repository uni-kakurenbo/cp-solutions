...
int main() {
    ...
    const int motin_limit = MOVE_LIMITS[m];
    debug(m, motin_limit);
    ...
    Field ans = initial_field;
    int score_max = 0;

    const int t = m >= 8 ? 2 : 1;
    for(int i=0; i < t; ++i) {
        const int time_limit_ms = TIME_LIMIT_MS * (i+1) / t;
        debug(time_limit_ms);

        Field pro_ans = solver.solve(0);
        int  pro_score_max = calc_score(pro_ans);
        ...
        if(score_max < pro_score_max) {
            score_max = pro_score_max;
            ans = move(pro_ans);
        }
    }
