...
struct point {
    ...
    inline point& clamp() { this->row = std::clamp(this->row, 0LL, lib::N-1), this->col = std::clamp(this->col, 0LL, lib::N-1); return *this; }
...
struct field : private uncopyable {
    ...
    std::array<cost_t,lib::S> strength = {};
...
struct solver : lib::uncopyable {
    ...
    void predict_strength() {
        ...
        REP(i, lib::S) {
            ...
            int neig = 4;
            if(p.row < this->D or p.row > lib::N-this->D) neig--;
            if(p.col < this->D or p.col > lib::N-this->D) neig--;
            ...
            pred[i] = w / s;
            // if(pred[i] < 50) pred[i] = 50;
            // if(pred[i] < 200) pred[i] = 100;
            end:;
        }

        // REP(i, lib::S) {
        //     const auto p = point { i };
        //     ld w = 2*pred[p.id()], s = 2;
        //     ITR(di, dj, DIRS8) {
        //         const point q = p + point { di, dj };
        //         if(!q) continue;
        //         w += pred[q.id()];
        //         s += 1;
        //     }
        //     this->prediction[p.id()] = w / s;
        // }
        std::copy(ALL(pred), std::begin(this->prediction));
    ...
    void connect_final() {
        ITR(i, this->connect_order) this->move(this->F->get_point(i));
    }
    ...
    void schedule_simulated() {
        ...
        std::vector costs(W+K, std::vector<lib::cost_t>(W+K));
        REP(i, W+K) REP(j, W+K) {
            costs[i][j] = this->find_path_simulated(this->F->get_point(i), this->F->get_point(j));
        }
        debug(costs);
        ...
        std::vector<lib::size_t> ids(this->F->K);
        std::iota(ALL(ids), this->F->W);
        std::sort(ALL(ids), [&dists](const auto &a, const auto &b) { return dists[a] < dists[b]; });
        debug(ids);

        this->connect_order = ids;

    }

    void move(lib::point start) {
        std::vector<lib::point> path;

        this->find_source_simulated(start, &path);
    ...
    lib::cost_t find_source_simulated(const lib::point start, std::vector<lib::point> *const path = nullptr) {
        using state = std::pair<lib::cost_t,lib::size_t>;
        ...
        while(not que.empty()) {
            cost_t d; size_t p;
            std::tie(d, p) = que.top(); que.pop();

            auto found = [this,&path,&prev,p](const point t) mutable -> bool {
            ...
            REP(i, this->F->W + this->F->K) if(found(this->F->get_point(i))) return dists[p];
