...
struct solution {
  ...
  public:
    ...
    std::vector<size_type> days;

    score_type score = std::numeric_limits<lib::score_type>::max();
    std::vector<score_type> scores;

    solution(const day_type D, const size_type E) noexcept : _D(D), edges(D), days(E), scores(D, std::numeric_limits<lib::score_type>::max()) {}
  ...
  public:
    ...
    inline auto apply(solution *const days, const neighbor_type neighbor) const noexcept {
        ...
        if(neighbor == 0) {
            ...
            const auto e = std::next(days->edges[d0].begin(), random(days->edges[d0].size()));
            days->edges[d1].insert(hint.e0 = *e), days->edges[d0].erase(e);
            days->days[hint.e0] = d1;
        ...
        if(neighbor == 1) {
            ...
            const auto e0 = std::next(days->edges[d0].begin(), random(days->edges[d0].size()));
        ...
        if(neighbor == 2) {
            day_type d0, d1;
            typename std::unordered_set<size_type>::iterator e0;

            do {
                do {
                    d0 = random(this->_in.D);
                    hint = { d0, -1 };
                } while(static_cast<day_type>(days->edges[d0].size()) <= 0);
                hint = { d0, -1 };

                e0 = std::next(days->edges[d0].begin(), random(days->edges[d0].size()));
                const auto& edge = this->_in.G.get_edge(*e0);

                const int node = random(2) == 0 ? edge.u : edge.v;
                const auto& adjs = this->_in.G.adjs(node);

                const int e1 = adjs[random(std::ranges::size(adjs))];
                hint.e1 = e1;

                hint.d1 = d1 = days->days[e1];
                debug(this->_in.G.get_node(node).x, this->_in.G.get_node(node).y, e0, e1, d0, d1);
            } while(d0 == d1 or std::ranges::ssize(days->edges[d1]) + 1 > this->_in.K);
