...
signed main() {
    ...
    function<void(int,int)> dfs = [&](int v, int p) {
        ...
        while(colors[v].count(color)) ++color;
        ITR(nv, G[v]) if(nv != p) while(colors[nv].count(color)) ++color;

        debug(color);

        ITR(nv, G[v]) {
            if(nv == p) continue;
            colors[v].emplace(color), colors[nv].emplace(color);
            ans[E[{nv,v}]] = color;
            dfs(nv, v);
            do { ++color; } while(colors[v].count(color));
        }
    };
    ...
    cout << reduce(ALL(ans), 0, [](int a, int b) { return max(a, b); }) << "\n";
    ITR(v, ans) cout << v << "\n";
