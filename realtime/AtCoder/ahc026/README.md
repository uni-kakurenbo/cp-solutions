...
For those who are not familiar with the Rust language environment, we have prepared a [pre-compiled binary for Windows](https://img.atcoder.jp/ahc026/lPQezTZx_windows.zip).
...
Note that this only draws the final state, so if all the boxes are successfully carried out, nothing is drawn.

You can also use a [web visualizer](https://img.atcoder.jp/ahc026/lPQezTZx.html?lang=en) which is more rich in features.
...
Rust言語の環境構築が面倒な方向けに、[Windows用のコンパイル済みバイナリ](https://img.atcoder.jp/ahc026/lPQezTZx_windows.zip)も用意してあります。
...
標準出力にはスコアを出力します。
ただし、ビジュアライザは最終状態のみを描画するため、全ての箱を運び出すことに成功した場合は何も描画されません。

より機能が豊富な[ウェブ版のビジュアライザ](https://img.atcoder.jp/ahc026/lPQezTZx.html?lang=ja)も利用可能です。
