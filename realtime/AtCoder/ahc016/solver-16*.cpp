  ...
  public:
    ...
    int predict(const std::string &H) const  {
        ...
        if(this->eps < 0.35) {
            ...
            int min = INF32;
            REP(t, degrees.size()) {
                if(this->trim_begin <= t and t <= this->trim_begin + this->trim_size) continue;
                if(diff[t] > *diff_min) continue;
                int cnt = 0;
                REP(i, G.size()) REPF(j, i+1, G.size()) {
                    // if(G.degree(i) < min_deg) continue;
                    // if(G.degree(j) < min_deg) continue;
                    if(G.has_edge(i,j) == 1) ++cnt;
                }
                debug(t);
                if(std::abs(cnt - t*(t-1)/2) < min) {
                    min = std::abs(cnt - t*(t-1)/2);
                    res = t;
                }
            }
        }
