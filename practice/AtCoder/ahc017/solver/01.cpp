...
struct graph {
    struct edge {
        ...
        edge(const size_type u, const size_type v, const cost_type w, const size_type id = -1) : from(u), to(v), cost(w), id(id) {}
    };
  ...
  public:
    graph(const size_type v = 0) : _edges(v) {}

    inline size_type size() const noexcept { return this->_edges.size(); }

    inline void add_edge(const size_type u, const size_type v, const cost_type w) noexcept {
        this->_edges[u].emplace_back(u, v, w, _edge_count);
        this->_edges[v].emplace_back(v, u, w, _edge_count);
        ++_edge_count;
    }
    ...
    inline auto compute_costs(const size_type s, const std::unordered_set<size_type>& ignored_edges = {}) const noexcept {
        ...
        while(not que.empty()) {
            ...
            for(const auto& e : this->_edges[u]) {
                if(ignored_edges.count(e.id)) continue;
...
struct evaluator {
  ...
  public:
    ...
    evaluator(const input* in) : _in(in), initial_costs(in->G.compute_cost_matrix()) {}
