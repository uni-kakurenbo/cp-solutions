...
struct Modifyer {
    ...
    Modifyer(int N, RandomEngine *random_engine, Evaluator *evaluator) : N(N), random_engine(random_engine), evaluator(evaluator) {}

    int modify(State *state) {
        const int type = state->board.history.empty() ? 0 : random_engine->range(3);
        ...
        if(type == 0) {
            ...
            Rectangle rectangle = found[random_engine->range(min((int)found.size(), 10))];
        ...
        if(type == 2) {
            ...
            Rectangle rectangle = found0[random_engine->range(found0.size())];
...
signed main() {
    ...
    Modifyer mdifyer(N, &random_engine, &evaluator);
    ...
    Score initial_score = evaluator.evaluate(initial_board);
    debug(initial_score);

    double start_temperature, end_temperature, temperature_diff;

    {
        constexpr double S0 = 700, S1 = 100;
        ...
        temperature_diff = end_temperature - start_temperature;
    ...
    constexpr double TIME_LIMIT_ENV = 1.0 / TIME_LIMIT;

    int loop_count = 0;
    while((clock() - start) < TIME_LIMIT) { // SA
        double time_rate = (clock() - start) * TIME_LIMIT_ENV;
        ...
        if(diff >= 0) {
            ...
            // print_answer(&cout, &clog, state);
        } else {
            double temperature = start_temperature + temperature_diff * time_rate;
            double probability = exp(diff / temperature);
            // debug(probability, random);
    ...
    { // Filling
       vector<Rectangle> found0;
        ...
        for(Rectangle &p : found0) {
            ...
            Rectangle rectangle = found1[random_engine.range((int)found1.size())];

            answer.board.draw(rectangle);
        }
    }

    debug(loop_count, answer);
