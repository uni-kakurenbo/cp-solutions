...
signed main() {
    int n, m; cin >> n >> m;
    vector<int> h(n), w(m);
    ITRR(v, h) cin >> v; ITRR(v, w) cin >> v;

    sort(ALL(h));

    vector<ll> sum0(n/2+1), sum1(n/2+1);

    REP(i, n/2) {
        sum0[i+1] = sum0[i] + h[i*2 + 1] - h[i*2];
        sum1[i+1] = sum1[i] + h[i*2 + 2] - h[i*2+1];
    }
    ...
    ll ans = INF64;
    ITR(x, w) {
        int ind = upper_bound(ALL(h), x) - h.begin();
        debug(ind, ind-ind%2);
        ind -= ind%2;
        ll diff = abs(x - h[ind]);
        diff += sum0[ind/2] + (*sum1.rbegin() - sum1[ind/2]);
        chmin(ans, diff);
    }
