...
signed main() {
    ...
    while(STORAGE.removed < N) {
        ...
        constexpr int DIV = 4;
        std::vector<std::pair<ld, std::vector<int>>> divdes;
        REP(div, lib::pow(M, DIV)) {
            std::vector<int> division;
            REP(i, DIV) {
                const int to = div / lib::pow(M, i) % M;
                if(to != STORAGE.stack(id)) division.emplace_back(to);
            }
            ...
            lib::run_length<int> rle(ALL(division));
            ITR(to, c, rle) {
                const int sz = std::ranges::size(copied.data[copied.stack(id)]);
                copied.move(copied.data[copied.stack(id)][lib::max(copied.pos(id) + 1, sz - num * c)], to);
        ...
        {
            auto minmax = std::ranges::minmax_element(divdes);
            debug(minmax.min);
            debug(minmax.max);
        }
        ...
        lib::run_length<int> rle(ALL(itr->second));
        ITR(to, c, rle) {
            const int sz = std::ranges::size(STORAGE.data[STORAGE.stack(id)]);
            STORAGE.move<true>(STORAGE.data[STORAGE.stack(id)][lib::max(STORAGE.pos(id) + 1, sz - num * c)], to);
