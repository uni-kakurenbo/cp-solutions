...
struct Tree {
    ...
    void dfs(vector<Candidate> *next_states, const bool one, Integer *priority, const Position search_range, const Integer dpath) {
        if (dpath == 0) {
            ...
            for (Rectangle &rectangle : found) {
                ...
                next_states->emplace_back(rectangle, this->node, *priority, next_score, next_hash, random_engine(-20, 20));
            }
        }
        else {
...
struct BeamSearch {
    ...
    Score run(vector<Rectangle> *answer, Integer width, const Position search_range, const Time appropriate_race_duration) const {
...
struct Solver {
    ...
    static inline constexpr Time calc_appropriate_race_duration(const Position N) {
    ...
    Score solve(vector<Rectangle> *answer) const {
        ...
        preliminary_answer.reserve(300);


        const UnsignedInteger basic_width = 2 * pow(61.0 / N, 8.15714246814);
        const UnsignedInteger basic_search_range = N / 4;

        UnsignedInteger width = basic_width;
        UnsignedInteger search_range = basic_search_range;

        clog << "Appropriate race duration: " << this->appropriate_race_duration << "\n";
        ...
        while(!timer.expired()) { // Beam Search
            ...
            Score score = beam_search.run(&preliminary_answer, width, search_range, this->appropriate_race_duration);
            debug(timer.elapsed());
            ...
            width = random_engine(8, 15) / 10.0 * basic_width;
            search_range = random_engine(8, 12) / 10.0 * basic_search_range;
        }
