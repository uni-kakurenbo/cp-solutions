...
struct cube_holder {
  private:
    std::unordered_map<size_t,polycube> _data;
    vector<size_t> _table;

  public:
    cube_holder(const size_t n = 0) : _table(n) {}

    inline void resize(const size_t n) {
        this->_table.resize(n), std::iota(ALL(this->_table), 0);
        this->_data.clear();
    }

    struct reference;

    // inline const polycube& operator[](const point3d p) const { return _data.at(_table[p]); }
    inline reference operator[](const point3d p) { return reference(this, p.id()); }
    ...
    struct reference {
      ...
      public:
        reference(cube_holder *const super, const size_t n) : _super(super), _n(n) {}
        reference(const reference& r) : _super(r._super), _n(r._n) {}


        const polycube& val() const { return _super->_data[_super->_table[this->_n]]; }

        polycube* operator->() { return &_super->_data[_super->_table[this->_n]]; }
        const polycube* operator->() const { return &_super->_data[_super->_table[this->_n]]; }

        explicit operator bool() const { return static_cast<bool>(_super->_data[_super->_table[_n]]); }


        reference& operator=(const reference& p) {
            _super->_data[_super->_table[this->_n]] = _super->_data[_super->_table[p._n]];
            return *this;
        }
        reference& operator=(const polycube& p) {
            _super->_data[_super->_table[this->_n]] = p;
            return *this;
        }

        reference& operator+=(const reference& p) {
            _super->_data[_super->_table[_n]] += _super->_data[_super->_table[p._n]];
            return *this;
        }
        friend reference operator+(const reference& p, const reference& q) { return reference(p) += q; }

        reference& operator+=(const polycube& p) {
            _super->_data[_super->_table[_n]] += p;
            return *this;
        }
        friend reference operator+(const reference& p, const polycube& q) { return reference(p) += q; }
        friend reference operator+(const polycube& p, const reference& q) { return reference(q) +=p; }
    };
  ...
  public:
    ...
    struct reference {
      ...
      public:
        ...
        inline size_t push(const cube_holder::reference& v) { return this->push(v.val()); }
        inline bool remove(const cube_holder::reference& v) { return this->push(v.val()); }
        inline size_t count(const cube_holder::reference& v) { return this->push(v.val()); }
...
void connect(const size_t n, const point3d p, const point3d q) {
    ...
    Put[n][p] += Put[n][q] + point3d{p} + point3d{q};
    Ps[n].push(Put[n][q] = Put[n][p]);
...
void solve() {
    ...
    DEBUG("--- connecting randomly ---");
    {
        REP(10 * Ps[0].count()) connect_randomly(0);
        REP(10 * Ps[1].count()) connect_randomly(1);
        debug(Put[0]);
        debug(Put[1]);
        debug(Ps);
    }
