...
struct solver : lib::uncopyable {
    ...
    size_t D = 10, H = (lib::N + D - 1) / D;

    std::vector<lib::pair<point>> plan;
    std::vector<point> convex_hull;

    lib::pair<cost_t> strengths[lib::S] = {};
    ...
    cost_t test_level(const section t) {
    ...
    void connect() {
        REP(i, this->F->W, this->F->W + this->F->K) this->connect(i);
    }

    void connect(const size_t start) {
        debug(start);
        ...
        auto estimate = [this,start,W,K](const size_t v) {
            cost_t dist = std::numeric_limits<cost_t>::max();
            REP(i, start+1, W+K) {
                if(W <= i && i <= start) continue;
                chmin(dist, this->get_leader(this->to_section(v)).dist_m(this->F->get_point(i)));
            }
            return dist*dist;
        };
        ...
        const size_t id = this->F->get_point(start).id();
        nodes[id] = id, que.emplace(id);

        while(not que.empty()) {
            node u = que.top(); que.pop();
            const section p = this->get_section(point{ u.id });

            REP(i, W+K) {
                if(W <= i && i <= start) continue;
                const section t = this->get_section(this->F->get_point(i));
                if(p == t) {
                    debug(i);
                    std::vector<lib::point> path;
                    while(true) {
                        path.push_back(point{ u.id });
                        if(u.prev < 0) break;
                        u = nodes[u.prev];
                        debug(point { u.id });
                    }
                    ...
                    REP(q, path.size()-1) this->move(path[q], path[q+1]);
                    this->move(*path.rbegin(), this->F->get_point(i));
                    return;
                }
            }
            ...
            ITR(di, dj, DIRS4) {
                ...
                const size_t v = this->get_leader(q).id();

                if(not this->is_valid(q)) continue;

                const cost_t w = this->F->is_reached(this->get_leader(q)) ? 0 : this->test_level(q);
    ...
    void move(point start, const point goal) {
        auto next_point = [goal](const lib::point cur) {
            ...
            ITR(di, dj, DIRS4) {
                ...
                if(chmin(min_dist, goal.dist_e(cand))) next = cand;
            }
            return next;
        };
    ...
    int destruct(const point p) {
        ...
        cost_t power = std::min(this->standard_power, this->test_level(this->get_section(p)));
