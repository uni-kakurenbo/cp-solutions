...
struct solver : lib::uncopyable {
    ...
    solver(lib::field *const F) : F(F), plan(F->W + F->K) {}
    ...
    void connect() {
        ...
        while(not que.empty()) {
            ...
            ITR(t, this->plan[s]) {
                ...
                if(this->F->broken.test(q.id())) continue;
                debug(s, t, p, q);
                this->move(p, q);
                que.push(t);
            }
    ...
    void move(lib::point start, const lib::point goal) {
        ...
        auto next_point = [goal](const lib::point cur, std::array<lib::pair<int>,4> dirs) {
            ...
            ITR(di, dj, dirs) {
                ...
                if(!cand) continue;
                lib::size_t dist = goal.dist_m(cand);
                if(chmin(min_dist, dist)) next = cand;
            }

            return next;
        };

        lib::cost_t limit = 1000;

        this->destruct(start);

        std::array<lib::pair<int>,4> dirs;
        std::copy(ALL(DIRS4), std::begin(dirs));

        std::deque<lib::point> path;

        while(true) {
            if(start == goal) break;
            path.push_back(start);

            start = next_point(start, dirs);
            // lib::cost_t mean = lib::average(cost.first, cost.second);
            const int res = this->destruct(start, limit);
            if(res == 2) {
                if(not path.empty()) start = path.front();
                std::shuffle(ALL(dirs), lib::rand);
            }

            if(path.size() > 5) path.pop_front();
        }

        this->destruct(goal);
    }

    lib::cost_t predict_strength(const lib::point p) {
        ...
        std::cout << "# predict: " << ave << std::endl;

        return std::clamp(ave, 10LL, 5000LL);
    }

    int destruct(const lib::point p, const lib::cost_t limit = 5000) {
        ...
        if(this->costs[p.id()].first > 0) power = 50;
