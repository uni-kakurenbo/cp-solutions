        ...
        for(auto group=groups.begin(); group!=last; ++group) {
            for(auto v=group->begin(); v!=group->end(); v++) {
                ...
                if(target->grid[i][j] != '0' && target->grid[i][j] != USED) {
                    for(int dir : CONNECT_DIRS) {
                        if(target->action_count_limit <= 0) return Evaluator::calc_score(&connected);
                        int pos = -1;
                        if(!target->can_connect(i, j, dir, &pos) || connected.same(*v, pos)) continue;
                        target->connect(i, j, dir);
                        connected.unite(*v, pos);
                    }
                }
            }
        }

        return Evaluator::calc_score(&connected);
    }
};
...
int main() {
...

    {
        ...
        int score = connector.connect_all(&tmp);
