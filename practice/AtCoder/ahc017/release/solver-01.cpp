...
struct graph {
  ...
  public:
    ...
    inline const auto& adjs(const size_type id) const noexcept { return this->_adjacencies[id]; }
    ...
    inline auto compute_costs(const size_type s, const std::vector<day_type>& days, const day_type ignore) const noexcept {
    ...
    inline auto compute_cost_matrix(const std::vector<day_type>& days, const day_type ignore) const noexcept {
        std::vector<std::vector<cost_type>> costs;
        costs.reserve(this->size());

        for(size_type v=0; v<this->size(); ++v) {
            costs.emplace_back(this->compute_costs(v, days, ignore));
        }
...
struct solution {
  ...
  public:
    std::vector<day_type> days;
    std::vector<size_type> count;

    score_type score = std::numeric_limits<lib::score_type>::max();
    std::vector<score_type> scores;

    solution(const day_type D, const size_type E) noexcept : _D(D), days(E), count(D), scores(D, std::numeric_limits<lib::score_type>::max()) {}
...
struct evaluator {
  ...
  public:
    ...
    void generate_samples() noexcept {
        ...
        constexpr std::size_t n = std::size(standards);

        std::array<pos_type, n> min_dist;
        std::array<size_type, n> closest;
    ...
    score_type compute_sampled_day_score(const std::vector<day_type>& days, const day_type ignore) const noexcept {
            score_type sum = 0;

            for(size_type i=0; i<this->_in.V; ++i) {
                auto costs = this->_in.G.compute_costs(i, days, ignore);
    ...
    score_type compute_sampled_score(const solution& schedule) const noexcept {
        ...
        for(day_type d=0; d<this->_in.D; ++d) {
            sum += this->compute_sampled_day_score(schedule.days, d);
        }
    ...
    score_type compute_sampled_score(const solution& schedule, std::vector<score_type> *const scores) const noexcept {
        ...
        for(day_type d=0; d<this->_in.D; ++d) {
            sum += scores->operator[](d) = this->compute_sampled_day_score(schedule.days, d);
        }
  ...
  public:
    ...
    inline auto apply(solution *const days, const neighbor_type neighbor) const noexcept {
        ...
        if(neighbor == 0) {
            size_type e0;
            do {
                e0 = random(this->_in.E);
                hint = { days->days[e0], (short)random(this->_in.D) };
            } while(hint.d0 == hint.d1 or days->count[hint.d1] + 1 > this->_in.K);
            hint.e0 = e0;
            days->days[e0] = hint.d1;
            --days->count[hint.d0], ++days->count[hint.d1];
        }
        ...
        if(neighbor == 1) {
            ...
            do {
                e0 = random(this->_in.E);
                e1 = (e0 + 1 + random(this->_in.E - 1)) % this->_in.E;
            } while(days->days[e0] == days->days[e1]);
            hint = { days->days[e0], days->days[e1] };
            hint.e0 = e0, hint.e1 = e1;
            std::swap(days->days[e0], days->days[e1]);
        }
        ...
        if(neighbor == 2) {
            ...
            do {
                do {
                    e0 = random(this->_in.E);
                } while(days->count[days->days[e0]] + 1 > this->_in.K);
                const auto& edge = this->_in.G.get_edge(e0);

                const int node = random(2) == 0 ? edge.u : edge.v;
                const auto& adjs = this->_in.G.adjs(node);

                e1 = adjs[random(std::ranges::size(adjs))];

                hint = { days->days[e0], days->days[e1] };
            } while(hint.d0 == hint.d1);
            hint.e0 = e0, hint.e1 = e1;

            days->days[hint.e1] = hint.d0;
            ++days->count[hint.d0], --days->count[hint.d1];
        }

        hint.s0 = this->_eval.compute_sampled_day_score(days->days, hint.d0) - days->scores[hint.d0];
        hint.s1 = this->_eval.compute_sampled_day_score(days->days, hint.d1) - days->scores[hint.d1];
    ...
    inline void rollback(solution *const days, const neighbor_type neighbor, const restore_hint& hint) const noexcept {
        ...
        if(neighbor == 0) {
            days->days[hint.e0] = hint.d0;
            ++days->count[hint.d0], --days->count[hint.d1];
        }
        else if(neighbor == 1) {
            days->days[hint.e0] = hint.d0;
            days->days[hint.e1] = hint.d1;
        }
        else {
            days->days[hint.e1] = hint.d1;
            --days->count[hint.d0], ++days->count[hint.d1];
    ...
    solution anneal(solution const &days, const timer &timer) {
        ...
        while(true) {
            ...
            if(score_diff > 0) {
                ...
                // assert(state.current().days == tmp.days);
                // assert(state.current().count == tmp.count);
            }
        }
...
signed main() {
    ...
    // lib::timer timer(100000);

    lib::input in(&std::cin);

    lib::evaluator evaluator(in);

    lib::solution initial_solution(in.D, in.E);

    constexpr lib::size_type INITIAL_SOLUTION_SIZE = 5;
...

    {
        ...
        for(lib::size_type i=0; i<INITIAL_SOLUTION_SIZE; ++i) {
            ...
            lib::solution temp_solution(in.D, in.E);
            for(lib::size_type e=0; e<in.E; ++e) {
                temp_solution.days[e] = days[e];
                ++temp_solution.count[days[e]];
            }
