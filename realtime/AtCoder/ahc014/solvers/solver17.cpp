...
template<class Engine = mt19937> struct RandomEngine {
  ...
  public:
    ...
    static constexpr UnsignedInteger min() const { return MIN; }
    static constexpr UnsignedInteger max() const { return MAX; }

    RandomEngine(UnsignedInteger seed = 3141592653UL) const {
        engine.seed(seed);
    };
...
struct BeamSearch {
    ...
    void run(vector<Rectangle> *answer, const Integer width) const {
