...
struct Line {
    ...
    inline UnsignedInteger slope_type() const {
...
struct LineCollector {
    ...
    stack<UnsignedInteger> pushed;

    inline bool overlapping_any(const Line &target) const {
        UnsignedInteger type = target.slope_type();
    ...
    inline UnsignedInteger size() const {
        return transform_reduce(
            this->buffers.cbegin(), this->buffers.cend(), 0,
            [](const UnsignedInteger a, const UnsignedInteger b) { return a + b; },
            [](const vector<Line> &buffer) -> UnsignedInteger { return buffer.size(); }
        );
    }
...
struct Board {
  public:
    ...
    inline void find_all_can_put_near_by(Integer px, Integer py, Integer d, vector<Rectangle>* found) {
...
struct Evaluator {
    ...
    inline Score evaluate(const Board &board) const {
        ...
        Integer weight_sum = transform_reduce(dots->cbegin(), dots->cend(), 0,
...
struct Tree {
    ...
    void dfs(vector<Candidate> *next_states, const bool one, Integer *priority, const Integer search_range, const Integer dpath) {
        if (dpath == 0) {
            ...
            for (const auto &[rectangle, ptr] : *children) {
                ...
                dfs(next_states, next_one, priority, search_range, dpath - 1);
...
struct BeamSearch {
    ...
    void run(vector<Rectangle> *answer, Integer width, const Integer search_range, const Time appropriate_race_duration) const {
        ...
        for (; ; ++loop_count) {
            ...
            Integer tmp = 0;
            tree.dfs(&next_states, true, &tmp, search_range, loop_count - tree.rank);
...
struct Solver {
    ...
    static constexpr float_t AVERAGE_ANSWER_LENGTH_COEFFICIENT = 5.10697058824;
    static constexpr float_t AVERAGE_ANSWER_LENGTH_CONSTANT_TERM = -104.775647059;

    static constexpr Time APPROPRIATE_RACE_DURATION_REVISION = -12;
    static constexpr Time APPROPRIATE_RACE_DURATION_MAX = 50;

    const Time appropriate_race_duration;

    static inline constexpr Time calc_appropriate_race_duration(const Integer N) {
        const float_t average_answer_ength =  AVERAGE_ANSWER_LENGTH_COEFFICIENT * N + AVERAGE_ANSWER_LENGTH_CONSTANT_TERM;
        dev_assert(average_answer_ength > 0);
        return min(APPROPRIATE_RACE_DURATION_MAX, Time(TIME_LIMIT_MS / average_answer_ength + APPROPRIATE_RACE_DURATION_REVISION));
    }

    Solver(const Integer N, const Integer M, const Grid &grid)
        : N(N), M(M), grid(grid), appropriate_race_duration(Solver::calc_appropriate_race_duration(N)) {}
    ...
    Score solve(vector<Rectangle> *answer) const {
        ...
        { // Beam Search
            ...
            const Integer width = 2 * pow(61.0 / N, 8.15714246814);
            const Integer search_range = N / 4;

            debug(width, search_range);
            debug(this->appropriate_race_duration);

            beam_search.run(answer, width, search_range, this->appropriate_race_duration);
        }
