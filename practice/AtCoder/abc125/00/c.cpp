...
signed main() {
    ...
    vector<int> gcd_l(n, a[0]), gcd_r(n, a.back());
    REP(i, n) {
        if(i == 0) gcd_l[0] = a[0];
        else gcd_l[i] = gcd(gcd_l[i-1], a[i]);
        if(i == 0) gcd_l[n-1] = a[n-1];
        else gcd_r[n-i-1] = gcd(gcd_r[n-i], a[n-i-1]);
    }
    debug(gcd_l, gcd_r);
    ...
    REP(i, n) {
        int g;
        if(i == 0) g = gcd_r[1];
        else if(i == n-1) g = gcd_l[n-2];
        else g = gcd(gcd_l[i-1], gcd_r[i+1]);
        chmax(ans, g);
    }
