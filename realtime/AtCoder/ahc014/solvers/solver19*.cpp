...
struct Board {
  public:
    ...
    Integer find_can_put0(const Integer px, const Integer py, vector<Rectangle>* found = nullptr) const {
        dev_assert(this->grid.is_valid(px, py));
        if(!this->grid.get(px, py)) return 0;
        ...
        for (Integer d = 0; d < 8; ++d) {
            ...
            if(found != nullptr) found->emplace_back(gx0, gy0, x00, y00, px, py, x01, y01);
            ++cnt;
        }
        return cnt;
    }

    Integer find_can_put1(const Integer px, const Integer py, vector<Rectangle>* found = nullptr) const {
        dev_assert(this->grid.is_valid(px, py));
        if(!this->grid.get(px, py)) return 0;
        ...
        for(int d=0; d<8; ++d) {
            ...
            if(
                ...
                if(tx2 == gx0 && ty2 == gy0) {
                    ...
                    if(tx3 == px && ty3 == py ) {
                        ...
                        if(found != nullptr) found->emplace_back(tx2, ty2, px, py, tx0, ty0, tx10, ty10);
                        ++cnt;
                    }
                }
            }
            ...
            if(
                ...
                if(tx2 == gx1 && ty2 == gy1) {
                    ...
                    if(tx3 == px && ty3 == py ) {
                        already.emplace(tx2, ty2);
                        if(found != nullptr) found->emplace_back(tx2, ty2, px, py, tx0, ty0, tx11, ty11);
                        ++cnt;
                    }
                }
            }
...
struct Modifier {
    ...
    inline Integer apply(State *state, const Rectangle &rectangle) const {
        ...
        state->drawable.erase(remove(state->drawable.begin(), state->drawable.end(), rectangle), state->drawable.end());
        return (
            state->board.find_can_put0(rectangle.x0, rectangle.y0, &state->drawable) +
            state->board.find_can_put1(rectangle.x0, rectangle.y0, &state->drawable)
        );
    }

    inline void restore(State *state, const Integer times, const Rectangle &rectangle) const {
        for(Integer i=0; i<times; ++i) state->drawable.pop_back();
        state->drawable.push_back(rectangle);
  ...
  public:
    ...
    void dfs(vector<Candidate>* next_states, const bool one, Integer* priority, const Integer dpath) {
        if (dpath == 0) {
            ...
            dev_assert(score == this->state.score);
            dev_assert(hash == this->state.hash);

            Integer cnt = 0;
            for (Rectangle &rectangle : this->state.drawable) {
                if(
                    !this->state.board.grid.get(rectangle.x0, rectangle.y0) &&
                    this->state.board.grid.get(rectangle.x1, rectangle.y1) &&
                    this->state.board.grid.get(rectangle.x2, rectangle.y2) &&
                    this->state.board.grid.get(rectangle.x3, rectangle.y3) &&
                    !this->state.board.lines.overlapping_any(Line{rectangle.x0, rectangle.y0, rectangle.x1, rectangle.y1}) &&
                    !this->state.board.lines.overlapping_any(Line{rectangle.x1, rectangle.y1, rectangle.x2, rectangle.y2}) &&
                    !this->state.board.lines.overlapping_any(Line{rectangle.x2, rectangle.y2, rectangle.x3, rectangle.y3}) &&
                    !this->state.board.lines.overlapping_any(Line{rectangle.x3, rectangle.y3, rectangle.x0, rectangle.y0}) &&
                    this->state.board.find_can_put0(rectangle.x2, rectangle.y2)
                ) {
            ...
            for (const auto& [rectangle, ptr] : *children) {
                ...
                Integer cnt = modifier.apply(&this->state, rectangle);

                dfs(next_states, next_one, priority, dpath - 1);

                if (!next_one) {
                    modifier.restore(&this->state, cnt, rectangle);
                }
            }
