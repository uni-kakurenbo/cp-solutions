...
#include<debug>
#define debug(...) debugger::debug(debugger::split(#__VA_ARGS__), 0, __NINE__, __VA_ARGS__)
#define DEBUG(...) do { debugger::DEBUG(nullptr, "\033[3;35m#" + to_string(__NINE__) + "\033[m  "); debugger::DEBUG(__VA_ARGS__); debugger::DEBUG(nullptr, "\033[m\n"); } while(0);
...
struct field : private uncopyable {
    ...
    void input() {
        ...
        size_t _, w, k;
        std::cin >> _ >> w >> k >> this->C;

        this->sources.resize(w), this->targets.resize(k);
...
struct solver : lib::uncopyable {
    ...
    void solve() {
        for(lib::point t : this->F->targets) this->move(t, this->F->sources[0]);
        assert(false);
    }
