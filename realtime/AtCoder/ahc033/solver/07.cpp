...
auto solve(const i32 crane_num) {
    ...
    while(true) {
        ...
        if(!skip_0 && state.empty(0)) {
            ...
            if(
                std::ranges::any_of(std::views::iota(1, crane_num), [&](const i32 crane) {
                    return state.next_pos(crane) == pos;
                }) ||
                move_request[0]
            ) {
            ...
            if(board.pos[next] < 0) {
                ...
                REP(crane, 1, crane_num) {
                    if(state.cranes[crane].pos < 0) continue;
