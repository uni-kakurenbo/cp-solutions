...
int main() {
    ...
    while((clock() - start) < TIME_LIMIT) {
        ...
        sort(buffer.begin(), buffer.end(), [](const State &a, const State &b) {
            return a.score == b.score
                       ? (a.gen == b.gen ? a.field.history.move.size() < b.field.history.move.size()
                                         : a.gen < b.gen)
                       : a.score > b.score;
        });
    ...
    State *ans = &buffer.front();
    {
        ...
        while(connector.connect_all(&tmp, &clusters1, true, 1));
        int score = Evaluator::calc_score(&clusters1);
        if(ans->score <= score) ans->score = score, ans->field = tmp;
        else {
            UnionFind clusters2(N*N);
            connector.connect_all(&ans->field, &clusters2, true);
        }
        // for(int i=0; i<K; ++i) connector.connect_all(&ans->field, &clusters, true, 100);
    }
