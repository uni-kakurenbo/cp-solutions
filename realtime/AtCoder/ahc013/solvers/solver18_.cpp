...
template<class OutStream, class LogStream = nullptr_t>
void print_answer(OutStream *out, History &res, __attribute__ ((unused)) LogStream  *log = nullptr, __attribute__ ((unused)) int gen = 0, __attribute__ ((unused)) int score = 0) {
    ...
    *out << connect->size() << endl;
    ...
    if constexpr (!is_same_v<LogStream, nullptr_t>) {
        *log << "---" << "\n";
        *log << "Generation: " << gen << "\n";
        *log << "Score: " << score << "\n";
    }
...
int main() {
...

    {
        ...
        #ifdef LOCAL_JUDGE
            print_answer(&of_log, tmp.history, &clog, 0, score);
        #else
            score_max += 0;
        #endif
    ...
    while((clock() - start) * 1000 < TIME_LIMIT_MS * CLOCKS_PER_SEC) {
        ...
        // debug(buffer, score_min);
            ...
            if(buffer.begin()->score > score_max) {
                ...
                connector.connect_all(&tmp);
                print_answer(&of_log, tmp.history, &clog, buffer.begin()->gen, score_max);
            }
        #endif
    ...
    clog << "Alpha: " << Alpha << "\n";
    clog << "Beta: " << Beta << "\n";
    print_answer(&cout, ans->field.history, &clog, ans->gen, ans->score);
