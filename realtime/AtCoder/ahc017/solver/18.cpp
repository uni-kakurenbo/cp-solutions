...
signed main() {
    ...
    {
        std::vector<std::pair<lib::graph::edge,lib::size_type>> edges;
        {
            const auto& es = in.G.edges();
            for(lib::size_type i=0; i<in.E; ++i) edges.emplace_back(es[i], i);
        }

        std::sort(edges.begin(), edges.end(), [&](const auto& a, const auto& b) {
            const lib::pos_type ax = in.G.get_node(a.first.v).x, ay = in.G.get_node(a.first.v).y;
            const lib::pos_type bx = in.G.get_node(b.first.v).x, by = in.G.get_node(b.first.v).y;
            return ax*ax + ay*ay < bx*bx + by*by;
        });

        for(lib::day_type i=0; i<in.E; ++i) {
            solution.edges[i%in.D].insert(edges[i].second);
        }
    }
