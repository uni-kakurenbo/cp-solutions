...
struct evaluator {
  ...
  public:
    ...
    void add_sample() noexcept {
        ...
        this->standards.pop_front();

        pos_type min_dist = std::numeric_limits<pos_type>::max();
        size_type closest = -1;

        for(size_type v=0; v<this->_in.V; ++v) {
            const auto& [x, y] = this->_in.G.get_node(v);
            pos_type dist = hypot2(standards.front().x - x, standards.front().y - y);
            if(dist < min_dist) min_dist = dist, closest = v;
        }

        this->samples.push_back(closest);
        debug(this->samples);
    }
    ...
    score_type compute_sampled_day_score(const std::unordered_set<size_type>& ignored_edges) const noexcept {
            ...
            return sum / this->samples.size()/this->samples.size();
    }
...
struct modifier {
  ...
  public:
    ...
    modifier(const input& in, evaluator *const eval) : _in(in), eval(eval) {}
    ...
    inline auto apply(solution *const days, const neighbor_type neighbor) const noexcept {
        ...
        {
            hint.s0 = this->eval->compute_sampled_day_score(days->edges[d0]) - days->scores[d0];
            hint.s1 = this->eval->compute_sampled_day_score(days->edges[d1]) - days->scores[d1];
...
struct annealer {
  private:
    ...
    modifier *const modify;

    double start_temp, end_temp, temp_change;

  public:
    annealer(modifier *const modify, const double start_temp, const double end_temp)
    ...
    solution anneal(solution const &days, const timer &timer) {
        ...
        while(true) {
            if(++loops % cycle == 0) {
                ...
                if(progress >= update_sample) {
                    this->modify->eval->add_sample();
                    update_sample += 0.1;
                }
