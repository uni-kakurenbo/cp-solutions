...
constexpr ll DETECTION_RANGE = 1000000;
...
struct Point {
    ...
    inline Pos dist(const Point &other) const {
        Pos dx = this->x - other.x;
        Pos dy = this->y - other.y;
        return dx*dx + dy*dy;
    }
...
RandomEngine random_engine(12345678);
...
signed main() {
    ...
    auto reset = [&]() {
        ...
        Point candidate;
        do {
            candidate.x = random_engine(-RADIUS, RADIUS);
            candidate.y = random_engine(-RADIUS, RADIUS);
        } while(not candidate.valid());
    ...
    REP(query, QUERIES) {
        ...
        if(type == 0) {
            ...
            debug(theta, inv_theta, abs(theta - inv_theta));
            if(abs(theta - inv_theta) < 0.12) {
                adv *= 0.40;
                --count;
            }
            ...
            while(not lod.valid()) {
                lod.x -= cos(theta)*adv / 50;
                lod.y -= sin(theta)*adv / 50;
            }
