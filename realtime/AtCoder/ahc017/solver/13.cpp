...
template<class T, class comp> using priority_queue = __gnu_pbds::priority_queue<T,comp>;
...
struct graph {
  ...
  public:
    ...
    inline auto compute_costs(const size_type s, const std::unordered_set<size_type>& ignored_edges = {}) const noexcept {
        ...
        priority_queue<state,std::greater<state>> que;

        priority_queue<state,std::greater<state>>::point_iterator node[this->size()];
        for(size_type i=0; i<this->size(); ++i) {
            node[i] = que.push({ std::numeric_limits<cost_type>::max(), i });
        }

        std::vector<cost_type> costs(this->size(), COST_INF);

        que.modify(node[s], { 0, s }), costs[s] = 0;
