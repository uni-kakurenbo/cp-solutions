...
void solve() {
...

    {
        auto a = A, b = B;
        REP(4*N) {
            A.push_back(a[uni::randi32() % N]);
            B.push_back(b[uni::randi32() % N]);
        }
    }
    ...
    std::vector<spair<i64>> points(m);
    REP(i, m) points[i] = { A[i], B[i] };

    ManhattanMinimumSpanningTree<i64> edges(points);
    edges.create_graph();
    debug(edges.graph.size());

    std::ranges::sort(edges.graph, [&](auto& a, auto& b) {
        return std::get<2>(a) < std::get<2>(b);
    });

    uni::disjoint_set uf(m);
