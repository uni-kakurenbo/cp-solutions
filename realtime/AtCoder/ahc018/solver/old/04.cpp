...
struct point {
    ...
    inline std::pair<size_t,size_t> _debug() const { return { this->row, this->col }; }
};
...
struct solver : lib::uncopyable {
    lib::field *const F;
    std::vector<std::pair<lib::size_t,lib::size_t>> plan;
    lib::dsu clusters;
    ...
    void move(lib::point start, const lib::point goal) {
        ...
        while(start != goal) {
            ...
            cost = this->destruct(start, std::clamp((cost.first*2 + cost.second) / 3, 10, 5000));
        }
    }

    std::pair<lib::cost_t,lib::cost_t> destruct(const lib::point p, lib::cost_t expected_strength = -1) {
        if(expected_strength < 0) expected_strength = 50 * std::log2(this->F->C) + 10;
        debug(expected_strength);
        dev_assert(10 <= expected_strength && expected_strength <= 5000);

        std::cout << "# expected: " << expected_strength << std::endl;

        // int cnt = 0;
        lib::cost_t p_cost = 10, cost = 10;

        while(not this->F->broken.test(p.id())) {
            const lib::cost_t power = expected_strength;// - std::pow(std::log10(expected_strength)), cnt);
        ...
        std::cout << "# expected: (" << p_cost << ", " << cost << "]" << std::endl;
