...
struct solver : lib::uncopyable {
    ...
    void destruct(size_t i, size_t j) {
        while(not this->F->broken.test(i*lib::N + j)) {
            lib::response result = this->F->excavate(i, j, this->power);

            if(result == lib::response::not_broken) this->power += this->F->C;
            if(result == lib::response::broken) this->power -= this->F->C;

            this->power = std::clamp(this->power, 10, 5000);
