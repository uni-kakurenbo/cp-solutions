...
void solve() {
    int n; cin >> n;
    lib::valarray<i64> a(n), t(n); input >> lib::views::zip(a, t);

    int q; cin >> q;
    lib::valarray<ll> x(q); input >> x;
    SegmentTree data(q, x.begin());

    REP(i, n) {
        if(t[i] == 1) data.add_val(0, q, a[i]);
        if(t[i] == 2) data.update_max(0, q, a[i]);
        if(t[i] == 3) data.update_min(0, q, a[i]);
    }

    REP(i, q) print << data.query_sum(i, i+1) << " ";
    print();
}
