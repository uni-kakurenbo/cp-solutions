...
struct Board {
  ...
  public:
    Board(const Grid *grid, const Point start) : _grid(grid), _start(start.id()), _terminator(-1) {
        this->_data.fill(Type::Empty);
        this->_score = this->_grid->value(start);
    }

    inline bool terminated(const Position point) const { return point == this->_terminator; }
    ...
    inline void set_terminator(const Point &point) { this->_terminator = point.id(); }
