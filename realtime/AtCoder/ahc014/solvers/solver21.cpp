...
struct LineCollector {
    ...
    stack<UnsignedInteger,vector<UnsignedInteger>> pushed;
    ...
    inline UnsignedInteger size() const {
        return accumulate(
            ...
            [](const UnsignedInteger acc, const vector<Line> &buffer) { return acc + buffer.size(); }
        );
    }
...
struct BeamSearch {
    ...
    void run(vector<Rectangle> *answer, Integer width, const Time appropriate_race_duration) const {
...
struct Solver {
    ...
    static constexpr Time APPROPRIATE_RACE_DURATION_MAX = 30;

    const float_t average_answer_ength;
    const Time appropriate_race_duration;

    Solver(const Integer N, const Integer M, const Grid &grid)
        : N(N), M(M), grid(grid),
          average_answer_ength(AVERAGE_ANSWER_LENGTH_COEFFICIENT * N + AVERAGE_ANSWER_LENGTH_CONSTANT_TERM),
          appropriate_race_duration(min(APPROPRIATE_RACE_DURATION_MAX, Time(TIME_LIMIT_MS / average_answer_ength + APPROPRIATE_RACE_DURATION_REVISION))) {
            dev_assert(this->average_answer_ength > 0);
    ...
    Score solve(vector<Rectangle> *answer) const {
        ...
        { // Beam Search
            ...
            beam_search.run(answer, width, this->appropriate_race_duration);
        }
