...
void solve() {
    ...
    std::valarray<ll> v(INF64, n);
    SegmentTree seg(n, std::ranges::begin(v));

    REP(q) {
        i32 l, r; input >> l >> r; --l;
        i64 c; input >> c;

        seg.update_min(l, r, c);
    }

    if(seg.query_max(0, n) >= INF64) {
        print(-1);
        return;
    }

    print(seg.query_sum(0, n));
}
