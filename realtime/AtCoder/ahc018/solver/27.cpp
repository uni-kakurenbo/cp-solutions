...
struct solver : lib::uncopyable {
    ...
    void connect() {
        REP(i, this->F->W, this->F->W + this->F->K) this->move(this->F->get_point(i));
    }

    void move(point start) {
        std::vector<point> path;
        this->find_path(start, &path);
    ...
    cost_t find_path(const point start, std::vector<point> *const path = nullptr) {
        ...
        auto estimate = [this](const size_t v) {
            if(this->F->is_reached(this->get_leader(this->to_section(v)))) return 0;

            cost_t dist = std::numeric_limits<cost_t>::max();

            REP(i, this->F->W + this->F->K) {
                const point p = this->F->get_point(i);
                if(not this->F->is_reached(p)) continue;

                chmin(dist, this->get_leader(this->to_section(v)).dist_m(p));
            }
        ...
        while(not que.empty()) {
            ...
            ITR(di, dj, DIRS4) {
                ...
                REP(i, this->F->W + this->F->K) {
                    const point t = this->F->get_point(i);
                    const section g = this->get_section(t);

                    if(not this->F->is_reached(t)) continue;
                    if(q != g) continue;
