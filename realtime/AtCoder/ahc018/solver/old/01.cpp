...
struct solver : lib::uncopyable {
    ...
    void schedule() {
        ...
        std::vector dp(1 << (W+K), std::vector<lib::size_t>(W+K, std::numeric_limits<lib::size_t>::max() >> 1));
        std::vector next(1 << (W+K), std::vector<lib::size_t>(W+K, -1));

        // house, water

        REP(S, 1 << (W+K)) REP(i, W) dp[S | (1 << i)][i] = 0;

        REP(S, 1 << (W+K)) REP(i, W+K) REP(j, W+K) {
            // debug(i, j, point(i), point(j), point(i).dist_m(point(j)));
            if(chmin(dp[S | (1 << i)][i], dp[S][j] + point(i).dist_m(point(j)))) {
                next[S][j] = i;
            }
        }

        auto path = [&](auto&& path, int S) -> void {
            REP(i, W+K) {
                if(next[S][i] >= 0) {
                    debug(next[S][i]);
                    this->plan.emplace_back(point(i), point(next[S][i]));
                    path(path, S | (1 << next[S][i]));
                }
            }
        };
        path(path, (1 << W) - 1);

        debug(this->plan.size(), this->plan);
    }
