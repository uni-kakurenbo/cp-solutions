...
struct Grid {
  ...
  public:
    ...
    inline void drop(const int x, const int y) {
        this->drop(converter->pair_to_id(x, y));
        return;
    }
...
signed main() {
    ...
    double start_temperature = 1000, end_temperature = 50;
    ...
    int initial_score = evaluator.evaluate(initial_board);

    // cin >> start_temperature >> end_temperature;

    State state(initial_score, initial_board);
    State answer = state;

    constexpr double TIME_LIMIT = TIME_LIMIT_MS * CLOCKS_PER_SEC / 1000;

    while((clock() - start) < TIME_LIMIT) {
        double exetime = (clock() - start) / TIME_LIMIT;
        ...
        if(type == 0) {
            ...
            int loop_count = 0;
            do {
                int x = random_engine.range(N), y = random_engine.range(N);
                state.board.find_rectangles(x, y, &found);
                ++loop_count;
            } while(found.empty() and loop_count < 100);
            if(found.empty()) continue;
            Rectangle &rectangle = found[random_engine.range(found.size())];
            next_board.draw(rectangle);
        }
        if(type == 1) {
            int times = random_engine.range(1, 7 / 2);
            if((int)next_board.history.size() < times) continue;
            for(int i=0; i<times; ++i) {
                Rectangle &last_drawn = next_board.history.back();
                next_board.grid.drop(last_drawn.x0, last_drawn.y0);
                next_board.history.pop_back();
                next_board.lines.pop(4);
            }
        }
        if(type == 2) {
            int times = random_engine.range(1, 7 / 2);
            ...
            for(int i=0; i<times; ++i) {
                ...
                do {
                    int x = random_engine.range(N), y = random_engine.range(N);
                    next_board.find_rectangles(x, y, &found);
                    ++loop_count;
                } while(found.empty() and loop_count < 30);
                if(found.empty()) continue;
                Rectangle &rectangle = found[random_engine.range(found.size())];
                next_board.draw(rectangle);
            }
        ...
        int next_score = evaluator.evaluate(next_board);

        int diff = next_score - state.score;

        if(diff >= 0) {
            debug(state.score, next_score);
            state = { next_score, next_board };
            answer = max(answer, state);
        } else {
            double temperature = start_temperature + (end_temperature - start_temperature) * exetime;
            // debug(temperature);
            double probability = exp(diff / temperature);
            double random = (double)rand() / RAND_MAX;
            // debug(type, probability, random);
