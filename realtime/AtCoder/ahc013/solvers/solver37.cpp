...
struct Connector {
    ...
    bool connect_clusters(Field *target, UnionFind *clusters, bool recording = false, int limit = INF32, UnionFind::Groups *_groups = nullptr) {
        ...
        if(_groups == nullptr) groups = new UnionFind::Groups, delete_flag = true;
        else groups = _groups;
        ...
        for(; group!=last && limit>0; ++group) {
            for(auto v=group->begin(); v!=group->end(); v++) {
                ...
                if(target->grid[i][j] == '0' || is_used(target->grid[i][j])) continue;
                for(int dir : CONNECT_DIRS) {
                    if(target->action_count_limit <= 0) {
                        if(delete_flag) delete groups;
                        return false;
                    }
                    int pos = -1;
...
struct Solver {
    ...
    Field gather(Grid *ref, UnionFind *clusters, int leader, int depth = INF32) {
        ...
        if(flag) clusters->unite(target, converter->pair_to_id(ti, tj));
...
int constexpr TIME_LIMIT_MS_1 = 2930;
int constexpr TIME_LIMIT_MS_2 = 2980;
double constexpr START_GATHERING = 0.8;
...
int main() {
    ...
    constexpr int START_GATHERING_TIME = (double)TIME_LIMIT_1 * START_GATHERING;

    debug(START_GATHERING_TIME);

    bool gathering_flag = false;

    while((clock() - start) < TIME_LIMIT_1) {
        gathering_flag = !gathering_flag && (clock() - start) >= START_GATHERING_TIME;
        ...
        for(int t=0; t<T; ++t) {
            ...
            for(int i=0; i<l; ++i) {
                ...
                for(int j=0; j<Alpha; ++j) {
                    Field res0 = solver.solve(frequency);

                    Field tmp0 = res0;
                    UnionFind clusters0(N*N);
                    UnionFind::Groups groups;
                    connector.connect_clusters(&tmp0, &clusters0, false, INF32, &groups);
                    int score0 = Evaluator::calc_score(&clusters0);

                    if(gathering_flag) {
                        int target = (*groups.begin())[0];

                        Field res1 = solver.gather(&tmp0.grid, &clusters0, target, 30);
                        Field tmp1 = res0;
                        UnionFind clusters1(N*N);
                        connector.connect_clusters(&tmp1, &clusters1);
                        int score1 = Evaluator::calc_score(&clusters1);

                        if(score1 > score0) score0 = score1, res0 = move(res1);
                    }

                    if(score0 > score_min) {
                        buffer.emplace_back(score0, gen + 1, res0);
                    }
                }
            }
...

    {
        ...
        while((clock() - start) < TIME_LIMIT_2 && level<(int)groups.size()) {
        ...
        if(score0 < score1) score0 = score1, tmp0 = move(tmp2);
