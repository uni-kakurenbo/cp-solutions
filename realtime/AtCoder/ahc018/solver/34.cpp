...
struct solver : lib::uncopyable {
    ...
    cost_t level(const section t) {
        ...
        lib::cost_t weights[] = { 50, 200, 11000 };

        bounds[0] = weights[0] = this->_bounds[0];
        bounds[1] = weights[1] = this->_bounds[1];
        weights[2] = this->_bounds[2];

        {
            ...
            REP(bound, std::begin(bounds), std::end(bounds)) {
                ...
                if(this->F->broken.test(p.id())) return this->levels[id] = *weight;
                prev = *bound;
                ++weight;
            }
        }

        return this->levels[id] = *std::rbegin(weights);
    }
...
signed main(int, char *argv[]) {
    ...
    sol.D = std::stoi(argv[1]);
    sol.H = (lib::N + sol.D - 1) / sol.D;
    sol.levels.assign(sol.H*sol.H, 0), sol.reached.assign(sol.H*sol.H, false);
    sol._bounds[0] = std::stoi(argv[2]);
    sol._bounds[1] = std::stoi(argv[3]);
    sol._bounds[2] = std::stoi(argv[4]);
