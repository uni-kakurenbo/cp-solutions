...
signed main() {
    ...
    REP(i, n) {
        ITRM(pos, v, dp[i]) {
            auto [x, y] = pos;
            if(!ng.count({x+a, y+b})) dp[i+1][{x + a, y + b}] += v;
            if(!ng.count({x+c, y+d})) dp[i+1][{x + c, y + d}] += v;
            if(!ng.count({x+e, y+f})) dp[i+1][{x + e, y + f}] += v;
        }
    }

    cout << accumulate(ALL(dp[n]), mint{}, [](mint acc, auto p) { return acc + p.$S; }) << ln;
