  ...
  public:
    ...
    inline auto apply(solution *const days, const neighbor_type neighbor) const noexcept {
        ...
        if(neighbor == 1) {
            ...
            dev_assert(days->edges[d0].size() <= _in.K and days->edges[d1].size() <= _in.K);
            if(
    ...
    solution anneal(solution const &days, const timer &timer) {
        ...
        while(true) {
            ...
            if(appling_probability > random.real()) {
                ...
                dev_assert(state.current().score == tmp.score);
                dev_assert(state.current().scores == tmp.scores);
                dev_assert(state.current().edges == tmp.edges);
            }
        }
...
signed main() {
    ...
    lib::annealer annealer(modifier, 1, 0);
