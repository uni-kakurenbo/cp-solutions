...
struct solver : lib::uncopyable {
    ...
    size_t D = 10, H = (lib::N + this->D - 1) / this->D;
    ...
    cost_t find_path(const point start, const point goal, std::vector<point> *const path = nullptr) {
        const section s = this->get_section(start), t = this->get_section(goal);
        ...
        auto estimate = [this,&goal](const size_t v) {
            return this->get_leader(this->to_section(v)).dist_e(goal);
        };
