...
struct point {
    pos_type x = -1, y = -1;
    inline std::pair<pos_type,pos_type> _debug() const noexcept { return { this->x, this->y }; }
};
...
struct evaluator {
  private:
    ...
    static auto generate_standards(const size_type n) noexcept {
        constexpr pos_type R = 500;
        const long double r = R * std::sqrt(1.0 / (2.0 * (1 + std::cos(2 * M_PI / n))));

        std::vector<point> res(n);
        for(size_type k=0; k<n; ++k) {
            res[k] = {
                static_cast<pos_type>(std::round(std::cos(2 * M_PI * k / n) * r)),
                static_cast<pos_type>(std::round(std::sin(2 * M_PI * k / n) * r))
            };
        }
  ...
  public:
    ...
    void generate_samples() noexcept {
        constexpr size_type n = 6;
        const auto standards = evaluator::generate_standards(n);
        debug(standards);

        std::vector<pos_type> min_dist(n, std::numeric_limits<pos_type>::max());
        std::vector<size_type> closest(n);
