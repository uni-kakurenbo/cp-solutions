...
signed main() {
    ...
    stack<State> buffer;
    buffer.emplace(initial_score, initial_board);
    State answer = buffer.top();

    int Alpha = max(50000 / (N*N), 1);
    // int Beta = max(57-N, 1);
    debug(Alpha);

    while((clock() - start) < TIME_LIMIT) {
        State current_state = buffer.top(); buffer.pop();
        ...
        for(Rectangle &rectangle : found) {
            ...
            uint32_t score = current_state.score + evaluator.weight(rectangle.x0, rectangle.y0);
            next_states.emplace_back(score, next_board);
        }
        sort(next_states.rbegin(), next_states.rend());

        if(next_states.front().score > answer.score) answer = next_states.front();

        for(int i=min(Alpha, (int)next_states.size()-1); i>=0; --i) {
            buffer.push(next_states[i]);
        }
        // debug(buffer);
        // debug(*buffer.begin());
    }
