  ...
  public:
    ...
    void generate_samples() noexcept {
        ...
        for(size_type v=0; v<this->_in->V; ++v) {
            const auto& [x, y] = this->_in->G.node(v);
...
signed main() {
    ...
    while(timer.remaining() >= unit_time) {
        ...
        std::vector<lib::day_type> days(in.E, -1);


        {
            std::unordered_set<lib::size_type> used;
            for(lib::day_type k=0; k<200; ++k) {
                lib::size_type v;
                do {
                    v = lib::random(in.E);
                } while(days[v] >= 0 or used.count(in.G.edge(v).u) or used.count(in.G.edge(v).v));
                days[v] = 0;
                for(const auto& e : in.G.adjacencies(in.G.edge(v).u)) {
                    used.insert(in.G.edge(e).u), used.insert(in.G.edge(e).v);
                }
                for(const auto& e : in.G.adjacencies(in.G.edge(v).v)) {
                    used.insert(in.G.edge(e).u), used.insert(in.G.edge(e).v);
                }
            }
        }
        for(lib::day_type d=1; d<in.D; ++d) {
            std::unordered_set<lib::size_type> used;
            for(lib::day_type k=0; k<(in.E-200)/in.D; ++k) {
                lib::size_type v;
                do {
                    v = lib::random(in.E);
                } while(days[v] >= 0 or used.count(in.G.edge(v).u) or used.count(in.G.edge(v).v));
        ...
        for(auto& day : days) if(day == -1) day = lib::random(1, in.D);
